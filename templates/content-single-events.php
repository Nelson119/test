<?php while (have_posts()) : the_post(); ?>
  <article <?php post_class(); ?>>
    <figure class="kv" style="background-image: url(<?php theme_asset('images/events/kv-1.png')?>)">
    </figure>
    <section class="container">
      <section class="row">
        <header>
          <span class="date"><span class="day">15</span> OCT 2015</span>
          <a class="signup visible-xs" href="javascript:">參加報名</a>
          <span class="category">
            <svg viewBox="0 0 13 13">
              <path d="M0,2.3L0,5c0,0.6,0.4,1.5,0.8,2l4.5,4.5C5.8,12,6.5,12,7,11.5l3.4-3.4c0.4-0.4,0.4-1.2,0-1.6
              L5.8,2c-0.4-0.4-1.3-0.8-2-0.8H1.2C0.5,1.2,0,1.7,0,2.3z M1.5,3.9c0-0.6,0.5-1.2,1.2-1.2s1.2,0.5,1.2,1.2C3.9,4.5,3.3,5,2.7,5
              S1.5,4.5,1.5,3.9z M12.1,7L6.2,1.2c0.6,0,1.5,0.4,2,0.8l4.5,4.5c0.4,0.4,0.4,1.2,0,1.6l-3.4,3.4c-0.4,0.4-0.9,0.4-1.4,0.2l4.1-4.1
              C12.2,7.4,12.2,7.2,12.1,7z"/>
            </svg>年會 Event
          </span>
          <h2>關於荊豔 <span class="sub">ABOUT - Dig More Stories About Chiayi<span></h2>
        </header>
        <aside class="col-lg-9 col-md-9 col-sm-9">
			<section class="entry-content">
				<aside class="intro">
					步步維艱的時代，<br>
					人人總想成為眾所『荊豔』的佼佼者<br>
					未知的年代，<br>
					你多久不再靜心聆聽內心的自己？<br>
					不再走出矗立的高樓？ 不再親身感受大地脈搏？<br>
					這次的年會主題為「荊豔」，擁有著三大元素：挖掘公民心視野、感受腳下記憶、凝聚內在力量<br>
					國立中正大學TEDxChungChengU團隊<br>
					將首次邀請來自三大領域的八位講者一同為您打造令您驚豔的饗宴<br>
					In the tough age, be the amazing one.<br>
				</aside>
				<hr>
				<h3>講者介紹<span class="sub">SPEAKERS</span></h3>
				<ul class="speakers row">
					<li>
					<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
						<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
					</a>
					<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
					<li>
						<a href="javascript:" style="background-image: url(<?php theme_asset('images/events/speaker-1.png')?>)">
							<span>嘉義市洪雅文化協會理事長，也是號稱「濁水溪以南最活躍的社運書店」的「洪雅書房」負責人。現年36歲的余國信衍然成為南臺灣居民共同心願的代言人。</span>
						</a>
						<p class="name">李宗霖</p>
						<p class="title">國際禮儀專家</p>
					</li>
				</ul>
				<hr>
				<h3>活動流程<span class="sub">PROGRAM</span></h3>
				<section class="calendar">
					<svg viewBox="0 0 22 22" >
						<path d="M5.4,5.2h0.3c0.5,0,0.9-0.4,0.9-0.9V2V1.1c0-0.5-0.4-0.9-0.9-0.9H5.4
							c-0.5,0-0.9,0.4-0.9,0.9V2v2.4C4.5,4.8,4.9,5.2,5.4,5.2z"/>
						<path d="M16.5,5.2h0.3c0.5,0,0.9-0.4,0.9-0.9V1.7V1.1c0-0.5-0.4-0.9-0.9-0.9h-0.3
							c-0.5,0-0.9,0.4-0.9,0.9v0.7v2.6C15.6,4.8,16,5.2,16.5,5.2z"/>
						<path d="M20.6,2h-2.2v2.6c0,0.9-0.7,1.3-1.6,1.3h-0.3c-0.9,0-1.6-0.7-1.6-1.6V2H7.3v2.4
							c0,0.9-0.7,1.6-1.6,1.6H5.4c-0.9,0-1.6-0.7-1.6-1.6V2H1.4C0.6,2,0,2.6,0,3.4v17c0,0.8,0.6,1.4,1.4,1.4h19.2
							c0.8,0,1.4-0.6,1.4-1.4v-17C22,2.6,21.4,2,20.6,2z M20.6,20.4H1.4l0-12.8h19.2L20.6,20.4C20.6,20.4,20.6,20.4,20.6,20.4z"/>
						<path d="M11.7,11.9h2.5c0.1,0,0.2-0.1,0.2-0.2V9.5c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C11.5,11.8,11.6,11.9,11.7,11.9z"/>
						<path d="M15.9,11.9h2.5c0.1,0,0.2-0.1,0.2-0.2V9.5c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C15.7,11.8,15.8,11.9,15.9,11.9z"/>
						<path d="M3.5,15.4H6c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2H3.5c-0.1,0-0.2,0.1-0.2,0.2
							v2.2C3.3,15.4,3.4,15.4,3.5,15.4z"/>
						<path d="M7.6,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2H7.6
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C7.4,15.4,7.5,15.4,7.6,15.4z"/>
						<path d="M11.7,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C11.5,15.4,11.6,15.4,11.7,15.4z"/>
						<path d="M15.9,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C15.7,15.4,15.8,15.4,15.9,15.4z"/>
						<path d="M6,16.5H3.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2H6c0.1,0,0.2-0.1,0.2-0.2
							v-2.2C6.2,16.6,6.1,16.5,6,16.5z"/>
						<path d="M10.1,16.5H7.6c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C10.3,16.6,10.2,16.5,10.1,16.5z"/>
						<path d="M14.3,16.5h-2.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C14.4,16.6,14.4,16.5,14.3,16.5z"/>
						<path d="M18.4,16.5h-2.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C18.6,16.6,18.5,16.5,18.4,16.5z"/>
					</svg>2014/05/15 星期六
				</section>
				<table>
					<tbody>
						<tr>
							<td>13：00～13：30</td>
							<td>報到入場</td>
						</tr>
						<tr>
							<td>13：30～14：05</td>
							<td>開場</td>
						</tr>
						<tr>
							<td>14：05～15：10</td>
							<td>凝聚內在的力量</td>
						</tr>
						<tr>
							<td>15：10～16：00</td>
							<td>挖掘公民心視野 Part1</td>
						</tr>
						<tr>
							<td>16：00～16：20</td>
							<td>中場休息</td>
						</tr>
						<tr>
							<td>17：00～18：30</td>
							<td>挖掘公民心視野 Part2</td>
						</tr>
						<tr>
							<td>18：30～19：00</td>
							<td>感受腳下記憶</td>
						</tr>
						<tr>
							<td>19：00～20：00</td>
							<td>閉幕</td>
						</tr>
						<tr>
							<td>19：00～20：00</td>
							<td>VIP與講者湖畔餐會時間</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							<th colspan="2">※ 下午13：00開始入場，除了VIP票有保留座位外，其餘座位依排隊順序自由入座</th>
						</tr>
					</tfoot>
				</table>
				<hr>
				<h3>活動地點<span class="sub">LOCATION</span></h3>
				<section class="location">
					<aside class="row">
						<svg viewBox="0 0 13 13">
						<path d="M6.5,0.5C4,0.5,2,2.6,2,5c0,1.2,0.5,2.3,1.2,3.1l3.3,4.4l3.3-4.4C10.5,7.3,11,6.2,11,5 C11,2.6,9,0.5,6.5,0.5z M6.5,6.4c-0.9,0-1.6-0.7-1.6-1.6c0-0.9,0.7-1.6,1.6-1.6c0.9,0,1.6,0.7,1.6,1.6C8.1,5.7,7.4,6.4,6.5,6.4z"/>
						</svg>
						<span class="place">
							<a class="address" href="javascript:">
								嘉義縣民雄鄉大學路168號
				            </a>
							國立中正大學 - 活動中心演藝廳
			            </span>
					</aside>
					<h5>交通資訊</h5>
					<ul class="unorder">
						<li>
							<p>公路</p>
							<ol class="numberic">
								<li>
									<p>1.經國道一號</p>
									由民雄交流道下，往民雄方向沿著民雄外環道(民新路)前行約1.9公里，過民雄陸橋後左轉進入省道台一線，前行約600公尺，右轉入大學路三段後，沿著大學路往前行駛約4.2公里即可抵達本校。</li>
								<li>
									<p>2.經國道三號</p>
									<ol class="numberic-alfa">
										<li>
											<p>(1)南下方向：</p>
											梅山交流道下→縣道162 (經過大林鎮公所)→左轉往民生路→往前行駛左轉大民北路→左轉到縣道106(大學路一段)→再行駛五分鐘即抵達中正大學
										</li>
										<li>
											<p>(2)北上方向：</p>
											竹崎交流道下→縣道166→民雄方向→再依照沿路指示牌，即可抵達中正大學
										</li>
									</ol>
								</li>
							</ol>
						</li>
						<li>
							<p>鐵路</p>
							<ol class="numberic">
								<li>
									1.搭乘火車至嘉義站，轉搭縣公車至本校，所需行車時間約40分鐘，經由省道往民雄由正門轉本校校園。校園內停靠三站，請在活動中心前上下車。</li>
								<li>
									2.搭乘火車至民雄站，出站後步行至民雄長壽會館轉乘嘉義縣公車。
								</li>
							</ol>
						</li>
						<li>
							<p>鐵路</p>
							<ol class="numberic">
								<li>
									1.搭乘火車至嘉義站，轉搭縣公車至本校，所需行車時間約40分鐘，經由省道往民雄由正門轉本校校園。校園內停靠三站，請在活動中心前上下車。</li>
								<li>
									2.搭乘火車至民雄站，出站後步行至民雄長壽會館轉乘嘉義縣公車。
								</li>
							</ol>
						</li>
						<li>
							<p>高鐵</p>
							由嘉義高鐵站搭乘計程車至本校，路程約30-40分鐘，費用為400至600元間。
						</li>
						<li>
							<p>客運</p>
							日統客運和台中客運皆行駛至本校校園
						</li>
						<li>
							<p>相關連結</p>
							<ol class="numberic">
								<li>
									<a href="javascript:">1.嘉義縣公車、日統客運、台中客運時刻表</a>
								</li>
								<li>
									<a href="javascript:">2.台鐵時刻表</a>
								</li>
								<li>
									<a href="javascript:">3.高鐵票價暨時刻表</a>
								</li>
							</ol>
						</li>
					</ul>
				</section>
				<hr>
				<h3>購票資訊<span class="sub">TICKET</span></h3>
				<table class="ticket">
					<thead>
						<tr>
							<td>票種</td>
							<td>票價</td>
							<td>說明</td>
							<td>售票時間</td>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td>中正大學校園票</td>
							<td>$220</td>
							<td>中正大學學生與教職員工(需出示相關證件)</td>
							<td>04/03 - 05/01</td>
						</tr>
						<tr>
							<td>一般票</td>
							<td>$260</td>
							<td>一般消費者適用</td>
							<td>04/03 - 05/01</td>
						</tr>
						<tr>
							<td>VIP票</td>
							<td>$400</td>
							<td>包含會後湖畔餐會、與講者互動機會</td>
							<td>04/03 - 05/01</td>
						</tr>
					</tbody>
				</table>
				<a href="javascript:" class="btn">立即購票</a>
				<hr>
				<h3>合作夥伴<span class="sub">PARTNERS</span></h3>
				<h5 class="mt35">主辦單位</h5>
				<ul class="holder row">
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/ccu.png')?>)">
						</figure>
						<p class="name">中正大學教學卓越計畫</p>
					</li>
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/ccu.png')?>)">
						</figure>
						<p class="name">中正大學職涯發展中心</p>
					</li>
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/tedxccu.png')?>)">
						</figure>
						<p class="name">TEDxChungChengU策展團隊</p>
					</li>
				</ul>
				<h5>協辦單位</h5>
				<ul class="holder row">
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/gamania.png')?>)">
						</figure>
						<p class="name">遊戲橘子關懷基金會</p>
					</li>
				</ul>
				<h5>特別感謝</h5>
				<ul class="holder row">
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/kk.png')?>)">
						</figure>
						<p class="name">Krispy Kreme (Taiwan)</p>
					</li>
					<li>
						<figure style="background-image: url(<?php theme_asset('images/events/housein.png')?>)">
						</figure>
						<p class="name">Livehouse</p>
					</li>
				</ul>
			</section>
		</aside>

		<aside class="col-lg-3 col-md-3 col-sm-3 w280">
			<aside class="pull">	
				<figure class="profile-picture" style="background-image: url(<?php theme_asset('images/events/profile-picture.png')?>)"></figure>
				<section class="row">
				<aside class="central">
					
					<svg viewBox="0 0 22 22" >
						<path d="M5.4,5.2h0.3c0.5,0,0.9-0.4,0.9-0.9V2V1.1c0-0.5-0.4-0.9-0.9-0.9H5.4
							c-0.5,0-0.9,0.4-0.9,0.9V2v2.4C4.5,4.8,4.9,5.2,5.4,5.2z"/>
						<path d="M16.5,5.2h0.3c0.5,0,0.9-0.4,0.9-0.9V1.7V1.1c0-0.5-0.4-0.9-0.9-0.9h-0.3
							c-0.5,0-0.9,0.4-0.9,0.9v0.7v2.6C15.6,4.8,16,5.2,16.5,5.2z"/>
						<path d="M20.6,2h-2.2v2.6c0,0.9-0.7,1.3-1.6,1.3h-0.3c-0.9,0-1.6-0.7-1.6-1.6V2H7.3v2.4
							c0,0.9-0.7,1.6-1.6,1.6H5.4c-0.9,0-1.6-0.7-1.6-1.6V2H1.4C0.6,2,0,2.6,0,3.4v17c0,0.8,0.6,1.4,1.4,1.4h19.2
							c0.8,0,1.4-0.6,1.4-1.4v-17C22,2.6,21.4,2,20.6,2z M20.6,20.4H1.4l0-12.8h19.2L20.6,20.4C20.6,20.4,20.6,20.4,20.6,20.4z"/>
						<path d="M11.7,11.9h2.5c0.1,0,0.2-0.1,0.2-0.2V9.5c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C11.5,11.8,11.6,11.9,11.7,11.9z"/>
						<path d="M15.9,11.9h2.5c0.1,0,0.2-0.1,0.2-0.2V9.5c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C15.7,11.8,15.8,11.9,15.9,11.9z"/>
						<path d="M3.5,15.4H6c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2H3.5c-0.1,0-0.2,0.1-0.2,0.2
							v2.2C3.3,15.4,3.4,15.4,3.5,15.4z"/>
						<path d="M7.6,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2H7.6
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C7.4,15.4,7.5,15.4,7.6,15.4z"/>
						<path d="M11.7,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C11.5,15.4,11.6,15.4,11.7,15.4z"/>
						<path d="M15.9,15.4h2.5c0.1,0,0.2-0.1,0.2-0.2v-2.2c0-0.1-0.1-0.2-0.2-0.2h-2.5
							c-0.1,0-0.2,0.1-0.2,0.2v2.2C15.7,15.4,15.8,15.4,15.9,15.4z"/>
						<path d="M6,16.5H3.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2H6c0.1,0,0.2-0.1,0.2-0.2
							v-2.2C6.2,16.6,6.1,16.5,6,16.5z"/>
						<path d="M10.1,16.5H7.6c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C10.3,16.6,10.2,16.5,10.1,16.5z"/>
						<path d="M14.3,16.5h-2.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C14.4,16.6,14.4,16.5,14.3,16.5z"/>
						<path d="M18.4,16.5h-2.5c-0.1,0-0.2,0.1-0.2,0.2v2.2c0,0.1,0.1,0.2,0.2,0.2h2.5
							c0.1,0,0.2-0.1,0.2-0.2v-2.2C18.6,16.6,18.5,16.5,18.4,16.5z"/>
					</svg>2014/05/15 星期六
				</aside>
				<aside class="central">
					<svg viewBox="0 0 13 13">
                		<path d="M6.5,0.5C4,0.5,2,2.6,2,5c0,1.2,0.5,2.3,1.2,3.1l3.3,4.4l3.3-4.4C10.5,7.3,11,6.2,11,5 C11,2.6,9,0.5,6.5,0.5z M6.5,6.4c-0.9,0-1.6-0.7-1.6-1.6c0-0.9,0.7-1.6,1.6-1.6c0.9,0,1.6,0.7,1.6,1.6C8.1,5.7,7.4,6.4,6.5,6.4z"/>
              		</svg>嘉義縣民雄鄉大學路168號
				</aside>
				<aside class="central noicon">
					國立中正大學活動中心演藝廳
				</aside>
				<aside class="central">
					<svg viewBox="0 0 13 13">
		                <path d="M6.5,2.6c-2.6,0-4.9,1.6-6,3.8c0,0.1,0,0.1,0,0.2c1,2.3,3.3,3.8,6,3.8s4.9-1.6,6-3.8
		                  c0-0.1,0-0.1,0-0.2C11.4,4.1,9.1,2.6,6.5,2.6z M6.5,9.2C5,9.2,3.8,8,3.8,6.5C3.8,5,5,3.8,6.5,3.8C8,3.8,9.2,5,9.2,6.5
		                  S8,9.2,6.5,9.2z"/>
		                <circle cx="6.5" cy="6.5" r="1.7"/>
					</svg>610
				</aside>
				<aside class="col-lg-9 col-md-9 col-sm-9 col-xs-10 name">
					<a href="javascript:" class="signup">
						我要參加報名
					</a>
				</aside>
				<aside class="col-lg-3 col-md-3 col-sm-3 col-xs-2">
				<a class="share">
				<svg viewBox="0 0 25 25">
				<path id="XMLID_434_" class="st0" d="M21.1,17.4c-1.2,0-2.5,0.7-3,1.8L7,13.5c0.3-0.6,0.4-1.1,0.4-1.7c0-0.3,0-0.6-0.1-0.8 l10.8-5.4c0.7,1.1,1.8,1.8,3,1.8c2.1,0,3.7-1.7,3.7-3.7S23.2,0,21.1,0s-3.7,1.7-3.7,3.7c0,0.4,0.1,0.8,0.1,1.1L6.9,10.2 C6.4,9.1,5.1,8.3,3.7,8.3C1.7,8.3,0,9.9,0,12s1.7,3.7,3.7,3.7c1.1,0,2.1-0.6,2.8-1.2l11.2,5.7c-0.1,0.4-0.1,0.7-0.1,1.1 c0,2.1,1.7,3.7,3.7,3.7s3.7-1.7,3.7-3.7C24.9,18.9,23.2,17.4,21.1,17.4z"/>
				</svg>
				</a>
				</aside>
				<nav class="share-buttons col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<ul>
				<li><a href="javascript:"><img src="<?php theme_asset('images/common/facebook.svg')?>"></a></li>
				<li><a href="javascript:"><img src="<?php theme_asset('images/common/twitter.svg')?>"></a></li>
				<li><a href="javascript:"><img src="<?php theme_asset('images/common/googleplus.svg')?>"></a></li>
				<li><a href="javascript:"><img src="<?php theme_asset('images/common/mail.svg')?>"></a></li>
				</ul>
				</nav>
				</section>
			</aside>

		</aside>


      </section>

    </section>
  </article>
<?php endwhile; ?>
